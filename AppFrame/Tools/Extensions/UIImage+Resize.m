...
- (UIImage *)resizeImageForUpload {
    if (self && [self isKindOfClass:[UIImage class]]) {
        if (self.size.width <= 750) {
            return self;
        } else {
            return [self resizeImageWithNewSize:CGSizeMake(750.f, 750.f / (self.size.width / self.size.height))];
        }
    }
     ...
     interpolationQuality:(CGInterpolationQuality)quality
{
    ...
    if ([self respondsToSelector:@selector(scale)])
        newRect = CGRectIntegral(CGRectMake(0, 0, newSize.width * scale, newSize.height * scale));
    else
